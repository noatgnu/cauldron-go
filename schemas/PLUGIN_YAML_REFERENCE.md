# CauldronGO Plugin Configuration

_Schema for CauldronGO plugin.yaml files_
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `name` | string | Yes | Unique identifier for the input (snake_case or camelCase) |
| `label` | string | Yes | Human-readable label displayed in the UI |
| `type` | string | Yes | Input type (file, text, number, boolean, select, etc.) |
| `required` | boolean | No | Whether this input is required (default: false) |
| `default` | any | No | Default value for the input |
| `description` | string | No | Help text displayed below the input |
| `placeholder` | string | No | Placeholder text for text inputs |
| `visibleWhen` | object | No | Conditional visibility based on other inputs |

**YAML Example with common properties:**
```yaml
- name: sample_count
  label: Sample Count
  type: number
