...
purity_cutoff <- ifelse(is.null(params$purity_cutoff), 0.6, as.numeric(params$purity_cutoff))
PeptideProphet_prob_cutoff <- ifelse(is.null(params$PeptideProphet_prob_cutoff), 0.7, as.numeric(params$PeptideProphet_prob_cutoff))
...
cat("Converting FragPipe data to MSstatsPTM format...\n")
data <- FragPipetoMSstatsPTMFormat(
        input = input_ptm,
