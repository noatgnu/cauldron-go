...
if (length(sample_cols_in_data) == 0) stop("No matching sample columns")
...
if (length(all_good_sample_cols) == 0) stop("No sample columns remaining")
...
pe <- readQFeatures(peptide_data_subset, quantCols = quant_cols, name = "peptideRaw")
...
if ("BioReplicate" %in% colnames(annotation)) colData_df_all$biorep <- factor(make.names(annotation$BioReplicate[match(all_good_sample_cols, annotation$Sample)]))
if ("Run" %in% colnames(annotation)) colData_df_all$run <- factor(annotation$Run[match(all_good_sample_cols, annotation$Sample)])
...
if (remove_shared_peptides) {
  shared_peptides <- grepl(";", rowData(pe[["peptideRaw"]])[[protein_col]]) | grepl(",", rowData(pe[["peptideRaw"]])[[protein_col]])
...
do_impute <- function(i, name) {
  if (!is.null(impute_method) && impute_method != "none") {
    pe <<- impute(pe, method = impute_method, i = i, name = name)
    return(name)
  }
...
do_norm <- function(i, name) {
  if (normalize_method != "none") {
    pe <<- normalize(pe, i = i, name = name, method = normalize_method)
    return(name)
  }
  pe <<- addAssay(pe, pe[[i]], name = name); return(name)
}
do_agg <- function(i, fcol, name) {
  peptide_counts <- table(rowData(pe[[i]])[[fcol]])
  proteins_to_keep <- names(peptide_counts)[peptide_counts >= filter_min_peptides]
  pe <<- pe[rowData(pe[[i]])[[fcol]] %in% proteins_to_keep, , ]
  summ_fun <- switch(summarization_method, "robust" = MsCoreUtils::robustSummary, "sum" = colSums, "mean" = colMeans, "median" = matrixStats::colMedians, MsCoreUtils::robustSummary)
  pe <<- pe[, colnames(pe[[i]])]
  pe <<- aggregateFeatures(pe, i = i, fcol = fcol, name = name, fun = summ_fun)
}
...
if (model_run_effect && "run" %in% colnames(colData(pe))) {
  if (length(unique(colData(pe)$run)) > 1) formula <- ~ condition + (1|run)
}
